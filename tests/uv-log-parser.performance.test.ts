...
describe('UvLogParser Performance Tests', () => {
  ...
  describe('Large Log Volume Handling', () => {
    ...
    it('should handle rapid HTTP/2 frame updates', () => {
      ...
      parser.parseLine('   uv_installer::preparer::get_wheel name=large-package==1.0.0, size=Some(1073741824), url="..."'); // 1GB
  ...
  describe('Memory Efficiency', () => {
    it('should not accumulate unlimited state data', () => {
      ...
      for (let i = 0; i < packageCount; i++) {
        parser.parseLine(`   uv_installer::preparer::get_wheel name=package${i}==1.0.0, size=Some(${1000 * i}), url="..."`);
        parser.parseLine(`Downloading package${i} (${i}KiB)`);
        parser.parseLine(`${i * 100}ms DEBUG h2::codec::framed_read received, frame=Data { stream_id: StreamId(${i}), flags: (0x1: END_STREAM) }`);
      }
    ...
    it('should cleanup completed transfers', () => {
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        parser.parseLine(`${i * 100}ms DEBUG h2::codec::framed_read received, frame=Data { stream_id: StreamId(${i}) }`);
      }
      
      // Complete half of them
      for (let i = 1; i <= 5; i++) {
        parser.parseLine(`${i * 200}ms DEBUG h2::codec::framed_read received, frame=Data { stream_id: StreamId(${i}), flags: (0x1: END_STREAM) }`);
      }
  ...
  describe('Concurrent Operations', () => {
    it('should handle multiple packages downloading simultaneously', () => {
      ...
      for (let i = 1; i <= concurrentDownloads; i++) {
        parser.parseLine(`   uv_installer::preparer::get_wheel name=concurrent${i}==1.0.0, size=Some(${1048576 * i}), url="..."`);
      ...
      for (let frame = 0; frame < 100; frame++) {
        for (let pkg = 1; pkg <= concurrentDownloads; pkg++) {
          parser.parseLine(`${frame * 10 + pkg}ms DEBUG h2::codec::framed_read received, frame=Data { stream_id: StreamId(${pkg}) }`);
        }
      }
